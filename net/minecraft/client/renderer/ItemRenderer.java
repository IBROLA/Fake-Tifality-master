...
public class ItemRenderer {
    ...
    private void renderArmInFirstPerson(AbstractClientPlayer clientPlayer, float p_178095_2_, float p_178095_3_) {
        ...
        if (handChams) instance.postHandRender();
        GlStateManager.enableCull();
    }
    ...
    private void func_178096_j(float equipProgress, float swingProgress) {
        Animations animations = Tifality.getInstance().getModuleManager().getModule(Animations.class);
        if (animations.coronaValue.get()) {
                    ...
                    switch (enumaction) {
                        ...
                        case BLOCK: {
                            ...
                            if (animations.isEnabled()) {
                                float eqiupProgress = animations.equipProgressProperty.getValue() ? f / animations.equipProgMultProperty.getValue().floatValue() : 0.0f;
                                GL11.glTranslated(animations.xPosProperty.getValue(), animations.yPosProperty.getValue(), animations.zPosProperty.getValue());
