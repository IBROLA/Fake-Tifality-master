...
import club.tifality.manager.event.impl.world.ScoreboardHeaderChangeEvent;
...
public class GuiIngame extends Gui {
    ...
    private static final Pattern domainPattern = Pattern.compile("(([a-zA-Z])|([a-zA-Z][a-zA-Z])|([a-zA-Z][0-9])|([0-9][a-zA-Z])|([a-zA-Z0-9][a-zA-Z0-9-_]{1,61}[a-zA-Z0-9]))\\.([a-zA-Z]{2,6}|[a-zA-Z0-9-]{2,30}\\.[a-zA-Z]{2,3})");
    /**
    ...
    public void renderGameOverlay(float partialTicks) {
        ...
        if (this.recordPlayingUpFor > 0) {
            ...
            if (l1 > 8) {
                ...
                this.getFontRenderer().drawString(this.recordPlaying, -this.getFontRenderer().getStringWidth(this.recordPlaying) / 2.0f, -4, l + (l1 << 24 & -16777216));
                GlStateManager.disableBlend();
                GL11.glPopMatrix();
            }
        }
        ...
        if (scoreobjective1 != null) {
            ...
            if (lastScoreboardHeader == null || !lastScoreboardHeader.equals(displayName)) {
                ...
                Tifality.getInstance().getEventBus().post(
    ...
    protected void renderScoreboard(ScoreObjective objective, ScaledResolution scaledRes) {
        ...
        for (Score score : collection) {
            ...
            String s = ScorePlayerTeam.formatPlayerName(scoreTeam, score.getPlayerName()) + ": " + EnumChatFormatting.RED + score.getScorePoints();
            i = Math.max(i, this.getFontRenderer().getStringWidth(s));
        }

        int i1 = collection.size() * MinecraftFontRenderer.FONT_HEIGHT;
        int j1 = scaledRes.getScaledHeight() / 2 + i1 / 3 + Hud.scoreBoardHeight.get().intValue();
        int k1 = 3;
        int l1 = scaledRes.getScaledWidth() - i - k1 - 2;
        int j = 0;

        for (Score score1 : collection) {
            ++j;
            ScorePlayerTeam scoreTeam = scoreboard.getPlayersTeam(score1.getPlayerName());
            String s1 = ScorePlayerTeam.formatPlayerName(scoreTeam, score1.getPlayerName());
            ...
            boolean skid = hud.isEnabled() && domainPattern.matcher(s1).find();
            GlStateManager.resetColor();
            if (skid) {
                RenderingUtils.drawOutlinedString("www.tifality.club", l1, k, hud.arrayListColorModeProperty.get() == Hud.ArrayListColorMode.RAINBOW ? RenderingUtils.getRainbow(hud.rainbowSpeed.getValue().intValue(), hud.rainbowWidth.getValue().intValue(), (int) (System.currentTimeMillis() / 15L)) : Hud.hudColor.get(), new Color(0, 0, 0).getRGB());
            } else {
            ...
            if (j == collection.size()) {
                ...
                this.getFontRenderer().drawString(s3, (float) l1 + (float) i / 2.0f - (float) this.getFontRenderer().getStringWidth(s3) / 2.0f, k - MinecraftFontRenderer.FONT_HEIGHT, 0xFFFFFFFF);
            }
        }
    }
